...
class coresolution():
    ...
    def get_connection(self, connectionName):
        def get_userid():
            request_uri = f'{self.scheme}://{self.ipaddress}:8080/connect/userinfo'
            response = requests.get(request_uri, headers=self.get_header(), verify=False)
            try:
        ...
        request_uri = f'{self.scheme}://{self.ipaddress}:8081/rest/api/job/v4.0/Connections/{connectionName}'
        response = requests.get(request_uri, headers=headers, verify=False)
        try:
            if response.status_code == 200:
                return response.json()["values"]
                ...
                parsed_response = json.loads(response.text)
                print(f"[-] Failed To Get Connection. | {parsed_response} | {response.status_code}")
                return -1
        except Exception as e:
