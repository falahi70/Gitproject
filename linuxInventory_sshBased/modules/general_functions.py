...
def load_config():
    try:
        config_handle = open("/CoreInspect/agents/linuxInventory/appsettings.json", "r", encoding="utf-8", errors="ignore")
    ...
    if len(ipList) > 0:
        for target_ip in ipList:
            try:
                ...
                output = sshConnection(target_ip["ip"], target_ip["username"], target_ip["password"], "cat /home/coreinspect/InventoryResult", target_ip["port"])
