...
def createCSVFile(path):
    ...
    for json_file in json_files:
        ...
        hostname_ipaddress = json_file.split('_')[1]
        ...
        for root_name, root_data in json_data.items():
            ...
            if (root_name== 'computerSystem'):
                max_scanid=computerSystem_max_scanid
            if (root_name== 'diskDrive'):
                max_scanid=diskDrive_max_scanid
            if (root_name== 'bios'):
            ...
            if (root_name== 'networkAdapterConfiguration'):
    ...
    try:
        for group_name, group_df in group_dataframes.items():
            ...
            group_df.columns = [col.lower() if isinstance(col, str) else col for col in group_df.columns]
            
            tableColumnNamequery = f"SELECT column_name FROM information_schema.columns WHERE table_name = '{group_name}'"
            
            tableColumnName = database_handle.fetch_sql(tableColumnNamequery)
            
            tableColumnName = [row[0] for row in tableColumnName]
            
            columns_to_drop = [col for col in group_df.columns if col not in tableColumnName]
