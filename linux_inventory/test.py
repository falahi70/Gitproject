...
def generate_userAccount_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["name"], \
                inventory_result["caption"], inventory_result["description"], inventory_result["fullname"], \
                inventory_result["status"], inventory_result["domain"], inventory_result["sid"]
        value = tuple(value)
        value = str(value)
        if values:
            values += f" , " + value
...
def generate_groupUser_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["groupcomponent"], \
                inventory_result["partcomponent"]
        value = tuple(value)
        value = str(value)
        if values:
            values += f" , " + value
...
def generate_group_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["name"], \
                inventory_result["caption"], inventory_result["description"], inventory_result["status"], \
                inventory_result["domain"], inventory_result["sid"]
        value = tuple(value)
        value = str(value)
        if values:
            values += f" , " + value
...
def generate_bootConfiguration_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["name"], \
                inventory_result["bootdirectory"]
        value = tuple(value)
        value = str(value)
        if values:
            values += f" , " + value
...
def generate_usbController_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["caption"], \
                inventory_result["manufacturer"]
        value = tuple(value)
        value = str(value)
        if values:
            values += f" , " + value
...
def generate_startupCommand_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["name"], \
                inventory_result["command"], inventory_result["location"], inventory_result["user"], inventory_result[
                    "usersid"]
        value = tuple(value)
        value = str(value)
        if values:
...
def generate_service_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["name"], \
                inventory_result["caption"], inventory_result["pathname"], inventory_result["acceptstop"], \
                inventory_result["startmode"], inventory_result["description"], inventory_result["displayName"], \
                inventory_result["servicetype"]
        value = tuple(value)
        value = str(value)
        if values:
            values += f" , " + value
...
def generate_product_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["name"], \
                inventory_result["caption"], inventory_result["description"], inventory_result["installdate"], \
                inventory_result["installlocation"], inventory_result["installsource"], inventory_result["vendor"], \
                inventory_result["version"], inventory_result["packagename"], inventory_result["packagecode"]
        value = tuple(value)
        value = str(value)
        if values:
            values += f" , " + value
...
def generate_processor_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["deviceid"], \
                ...
                inventory_result["numberoflogicalprocessors"], inventory_result["partnumber"], inventory_result[
                    "processorid"], inventory_result["socketdesignation"], inventory_result["threadcount"], \
                inventory_result["virtualizationfirmwareenable"], inventory_result["numberofcores"]
        value = tuple(value)
        value = str(value)
        if values:
            values += f" , " + value
...
def generate_physicalMemory_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["capacity"], \
                ...
                inventory_result["manufacturer"], inventory_result["partnumber"], inventory_result["serialnumber"], \
                inventory_result["speed"], inventory_result["tag"]
        value = tuple(value)
        value = str(value)
        if values:
            values += f" , " + value
...
def generate_networkAdapterConfiguration_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["ipaddress"], \
                inventory_result["dnsdomain"], inventory_result["dnshostname"], inventory_result["interfaceindex"], \
                inventory_result["macaddress"], inventory_result["description"], inventory_result["mtu"], \
                inventory_result["ipsubnet"]
        value = tuple(value)
        value = str(value)
        if values:
            values += f" , " + value
...
def generate_logicalDisk_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["name"], \
                inventory_result["caption"], inventory_result["description"], inventory_result["volumename"], \
                inventory_result["size"], inventory_result["filesystem"]
        value = tuple(value)
        value = str(value)
        if values:
            values += f" , " + value
...
def generate_diskDrive_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["manufacturer"], \
                inventory_result["deviceid"], inventory_result["description"], inventory_result["size"], \
                inventory_result["pnpdeviceid"], inventory_result["medialoaded"]
        value = tuple(value)
        value = str(value)
        if values:
            values += f" , " + value
...
def generate_bios_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["caption"], \
                inventory_result["description"], inventory_result["manufacturer"], inventory_result["version"], \
                inventory_result["releasedate"]
        value = tuple(value)
        value = str(value)
        if values:
            values += f" , " + value
...
def generate_computerSystem_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["name"], \
                inventory_result["dnshostname"], inventory_result["domain"], inventory_result["workgroup"], \
                inventory_result["totalphysicalmemory"], inventory_result["systemskunumber"], inventory_result[
                    "primaryownername"], inventory_result["numberofprocessors"], inventory_result[
                    "numberoflogicalprocessors"], inventory_result["osMachineId"], inventory_result["manufacturer"], \
                inventory_result["hypervisorpresent"]
        value = tuple(value)
        value = str(value)
        if values:
            values += f" , " + value
...
def generate_operatingSystem_query(ip, hostname, inventory_results):
    ...
    for inventory_result in inventory_results:
        value = target_command, scannedip, osfamily, hostname, scanid, createdtime, inventory_result["name"], \
                inventory_result["caption"], inventory_result["buildNumber"], inventory_result["currentTimeZone"], \
                inventory_result["installdate"], inventory_result["lastBootUpTime"], inventory_result["osArchitecture"], \
                inventory_result["version"], inventory_result["serialNumber"]
        value = tuple(value)
        value = str(value)
        if values:
            values += f" , " + value
...
def insert_data(device_target):
    ...
    try:
        if "userAccount" in inventory_result and (bool(inventory_result["userAccount"])):
            userAccount = generate_userAccount_query(device_target["ipAddress"], device_target["hostName"],
                                                     inventory_result["userAccount"])
            #print(userAccount)
        else:
            userAccount = ""

        if "groupUser" in inventory_result and (bool(inventory_result["groupUser"])):
            groupUser = generate_groupUser_query(device_target["ipAddress"], device_target["hostName"],
                                                 inventory_result["groupUser"])
        else:
            groupUser = ""

        if "group" in inventory_result and (bool(inventory_result["group"])):
            group = generate_group_query(device_target["ipAddress"], device_target["hostName"],
                                         inventory_result["group"])
        else:
            group = ""

        if "bootConfiguration" in inventory_result and (bool(inventory_result["bootConfiguration"])):
            bootConfiguration = generate_bootConfiguration_query(device_target["ipAddress"], device_target["hostName"],
                                                                 inventory_result["bootConfiguration"])
        else:
            bootConfiguration = ""

        if "usbController" in inventory_result and (bool(inventory_result["usbController"])):
            usbController = generate_usbController_query(device_target["ipAddress"], device_target["hostName"],
                                                         inventory_result["usbController"])
        else:
            usbController = ""

        if ("startupCommand" in inventory_result) and (bool(inventory_result["startupCommand"])):
            startupCommand = generate_startupCommand_query(device_target["ipAddress"], device_target["hostName"],
                                                           inventory_result["startupCommand"])
        else:
            startupCommand = ""

        if "service" in inventory_result and (bool(inventory_result["service"])):
            service = generate_service_query(device_target["ipAddress"], device_target["hostName"],
                                             inventory_result["service"])
        else:
            service = ""

        if "product" in inventory_result and (bool(inventory_result["product"])):
            product = generate_product_query(device_target["ipAddress"], device_target["hostName"],
                                             inventory_result["product"])
        else:
            product = ""

        if "processor" in inventory_result and (bool(inventory_result["processor"])):
            processor = generate_processor_query(device_target["ipAddress"], device_target["hostName"],
                                                 inventory_result["processor"])
        else:
            processor = ""

        if "physicalMemory" in inventory_result and (bool(inventory_result["physicalMemory"])):
            physicalMemory = generate_physicalMemory_query(device_target["ipAddress"], device_target["hostName"],
                                                           inventory_result["physicalMemory"])
        else:
            physicalMemory = ""

        if "networkAdapterConfiguration" in inventory_result and ( bool(inventory_result["networkAdapterConfiguration"])):
            networkAdapterConfiguration = generate_networkAdapterConfiguration_query(device_target["ipAddress"],
                                                                                     device_target["hostName"],
                                                                                     inventory_result[
        ...
        if "logicalDisk" in inventory_result and (bool(inventory_result["logicalDisk"])):
            logicalDisk = generate_logicalDisk_query(device_target["ipAddress"], device_target["hostName"],
                                                     inventory_result["logicalDisk"])
        else:
            logicalDisk = ""

        if "diskDrive" in inventory_result and (bool(inventory_result["diskDrive"])):
            diskDrive = generate_diskDrive_query(device_target["ipAddress"], device_target["hostName"],
                                                 inventory_result["diskDrive"])
        else:
            diskDrive = ""
        ...
        if "computerSystem" in inventory_result and (bool(inventory_result["computerSystem"])):
            computerSystem = generate_computerSystem_query(device_target["ipAddress"], device_target["hostName"],
                                                           inventory_result["computerSystem"])
            print(computerSystem)
        else:
            computerSystem = ""

        if "operatingSystem" in inventory_result and (bool(inventory_result["operatingSystem"])):
            operatingSystem = generate_operatingSystem_query(device_target["ipAddress"], device_target["hostName"],
                                                             inventory_result["operatingSystem"])
        else:
            operatingSystem = ""
