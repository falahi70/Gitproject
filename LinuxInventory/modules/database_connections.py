...
class databaseConnection:
    ...
    def bulk2db(self, schema_name, table_name, data_list,csv_columns):
        ...
        tableColumnNamequery = f"SELECT column_name FROM information_schema.columns WHERE table_name = '{table_name}'"
        cursor.execute(tableColumnNamequery)
        return_results = [item for item in cursor.fetchall()]
        tableColumnName = [row[0] for row in return_results]
        remove_non_matching_columns(data_list,tableColumnName)
        csv_headers_array = get_csv_headers(data_list)
    ...
    def get_max_scanidnohost(self, schema_name, table_name):
        query = f"select MAX(scanid) from \"{schema_name}\".\"{table_name}\";"
        return self.fetch_sql(query)[0][0]
