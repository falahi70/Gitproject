...
try:
    ...
    os.chdir(f'C:\\Users\\{currentUser}\\CoreInspectAgent\\PowerVM')
...
def Get_Creds(CredName, coresolution_handle):
    global VUsername
    global VString

    try:
        coresolution_handle.authenticate()
        credential_handle = coresolution_handle.get_credential(CredName)
        VUsername = credential_handle["username"]
        VString = credential_handle["password"]
        return False
    except Exception as e:
        print(f"[-] Failed to get credential.\n[Err] {e}")
        return False
...
coresolution_handle = coresolution(
    config["coreInspectConnectionSchema"],
    config["coreInspectIpAddress"],
    config["coreInspectUsername"],
    config["coreInspectPassword"],
)
coresolution_handle.authenticate()
Get_Creds(options.credential, coresolution_handle)
    ...
    if host_data:
        ...
        for i in range(0, len(final_list)):
            ...
            val = (data["vHostCpuModel"], data["vHostCpuMhz"], data["vHostNumCpu"], data["vHostCoresPerCPU"],
                   ...
                   data["vHostObjectID"], data["vHostServiceTag"], data["ip"], data["vmnic0"] if "vmnic0" in data.keys() else "", data["vmnic1"] if "vmnic1" in data.keys() else "",
