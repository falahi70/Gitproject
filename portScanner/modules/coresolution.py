...
CORESOLUTIONIP = config["coreInspectIpAddress"]
jwt_token = ""
token_status = ""
job_detail_staus = ""
job_detail_count = ""
...
def ConvertIP(input,type="decimal"):
    if type == "ip":
        return int(ipaddress.ip_address(input))
    return ipaddress.ip_address(input).__str__()

def Get_JWT(Username, Password, ipAddrses=CORESOLUTIONIP):
    def authorize(Username, Password):
        headers = {
            ...
            'Content-Type': 'application/x-www-form-urlencoded',
            'Content-Length': '72',
            'Origin': f'https://{ipAddrses}',
            'Connection': 'close',
            'Referer': f'https://{ipAddrses}/',
        }
        data = rf'grant_type=password&client_id=ro&username={Username}&password={Password}'
        response = requests.post(f'https://{ipAddrses}:8080/connect/token', headers=headers, data=data, verify=False)
        jwt_token_response = json.loads(response.text)
        jwt_token = jwt_token_response["access_token"]
        token_status = response.status_code
        return jwt_token, token_status, jwt_token_response
...
def CreateJob(Token, cpl):
    headers = {
        'Host': f'{CORESOLUTIONIP}:8081',
        'User-Agent': HTTPUserAgent,
        'Accept': '*/*',
        'Accept-Language': 'en-US,en;q=0.5',
        'Accept-Encoding': 'gzip, deflate',
        ...
        'X-Requested-With': 'XMLHttpRequest',
        'Content-Length': '121',
        'Origin': f'https://{CORESOLUTIONIP}',
        'Connection': 'close',
        'Referer': f'https://{CORESOLUTIONIP}/',
    }
    
    cpl = fr"| search resourceType = \"networkNode\" | fields ipAddress"
    cpl = cpl if "," not in cpl[-1] else cpl[:-1]
    print_cpl = cpl.replace(r'\"','"')
    print_cpl = print_cpl.replace(r'\n','')
    final_cpl = rf'{{"cpl":"{cpl}","timeRange":null,"moduleName":"Resources","searchPoint":"ApplicationModule","runId":""}}'
    print   (final_cpl)
    response = requests.post(f'https://{CORESOLUTIONIP}:8081/rest/api/job/v4.0/Jobs', headers=headers, data=final_cpl,
                             verify=False)
    job_response = json.loads(response.text)
    # print(job_response)
    return job_response, response.status_code

def Get_Result_Key(JobID, Token):
    headers = {
        'Host': f'{CORESOLUTIONIP}:8081',
        'User-Agent': HTTPUserAgent,
        'Accept': '*/*',
    ...
    response = requests.get(
        f'https://{CORESOLUTIONIP}:8081/rest/api/job/v4.0/Jobs/{JobID}/Results?pageSize=999999',
        headers=headers, params=params, verify=False)
    job_key_response = json.loads(response.text)
    job_key_statuscode = response.status_code
    return job_key_statuscode, job_key_response

def Get_Job_Data(job_id, Token):
    headers = {
        'Host': f'{CORESOLUTIONIP}:8081',
        'User-Agent': HTTPUserAgent,
        'Accept': '*/*',
    ...
    response = requests.get(f'https://{CORESOLUTIONIP}:8081/rest/api/job/v4.0/Jobs/{job_id}',
                            headers=headers, verify=False)
    job_detail_response = json.loads(response.text)
    # print(f'Job Response: {job_detail_response["values"]}')
    try:
        job_detail_staus = job_detail_response["values"]["status"]
        job_detail_count = job_detail_response["values"]["count"]
        try:
            failureMessage = job_detail_response["values"]["failureMessage"]
        except:
            failureMessage = "NotDefined"
    except Exception as e:
        print(f"[+] Failed to get job data: {e}")
        job_detail_staus = "Completed"
        job_detail_count = 0
    return job_detail_staus, job_detail_count, failureMessage

def FetchResourceType(username="CoreInspect", password="CoreInspect"):
	
    jwt_token, token_status = Get_JWT(username, password)
    job_response, job_status = CreateJob(jwt_token, cpl="")
    job_valid_staus = True
    while job_valid_staus:
        if str(job_status) == "201":
            job_valid_staus = False
        else:
            print("Job Creation Failed, Generating New Token And Trying Again In 3sec...")
            print(f"Failure Reason: {job_response}")
            if "Invalid Cpl is Provided" in str(job_response):
                return 0
            time.sleep(3)
            Get_JWT(config["coreInspectUsername"], config["coreInspectPassword"])
            job_response, job_status = CreateJob(jwt_token, cpl="")
    job_id = job_response["values"]["id"]
    job_status = job_response["values"]["status"]
    time.sleep(3)
    job_detail_staus, job_detail_count, job_failed_message = Get_Job_Data(job_id, jwt_token)
    rescan_job_status = True

    while rescan_job_status:
        if job_detail_staus == "Completed":
            rescan_job_status = False
        elif job_detail_staus == "Failed":
            print(f"[-] Failed to execute job.")
            print(f"Status: {job_detail_staus}")
            print(f"Reason: {job_failed_message}")
            rescan_job_status = False
            job_detail_count = 0
        else:
            print("Job Not Finished Yet, Waiting 3 More Seconds...")
            print(f"Job Status: {job_detail_staus}")
            time.sleep(3)
            job_detail_staus, job_detail_count, job_failed_message = Get_Job_Data(job_id, jwt_token)

    if int(job_detail_count) < 1:
        print("No New Resource Has Been Added, Exiting...")
        print(f"Job Detail Status: {job_detail_staus}")
        print(f"Job Count: {job_detail_count}")
        print(f"Job Failed Message: {job_failed_message}")
        return 0
    else:
        pass
        # print(f"Job Count: {job_detail_count}")

    job_key_statuscode, job_key_response = Get_Result_Key(job_id, jwt_token)
    if int(job_key_statuscode) != 200:
        print("No Valid Job Key Has Been Fetched, Exiting...")
        return 0
    returned_data = []
    
    ip_list = []
    for fetched_result in job_key_response["results"]:
        long_ip = int(fetched_result["ipAddress"])
        str_ip = ConvertIP(long_ip)
