...
class coresolution():
    ...
    def get_snmp_credential(self, connection_name):
        def get_connection_details(connection_name):
            request_uri = f'{self.scheme}://{self.ipaddress}:8081/rest/api/job/v4.0/Connections/{connection_name}'
            response = requests.get(request_uri, headers=self.get_header(), verify=False)
            try:
                if response.status_code == 200:
                    return response.json()["values"]
        ...
        connection_information = get_connection_details(connection_name)
        ...
        for option in connection_handle_options:
