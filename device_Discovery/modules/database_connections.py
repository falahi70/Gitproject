...
class databaseConnection:
    ...
    def truncate_scanid(self, schema_name, table_name, latest_num, condition=""):
        print("[!!] ONLY PRINTING")
        if not condition:
            get_max_scanid_query = f"select MAX(scanid) from \"{schema_name}\".\"{table_name}\""
        else:
            get_max_scanid_query = f"select MAX(scanid) from \"{schema_name}\".\"{table_name}\" {condition}"
        print(f"[*] get_max_scanid_query: {get_max_scanid_query}")
        # max_scanid = self.fetch_sql(get_max_scanid_query)[0][0]
        max_scanid = 100

        scanid_condition = int(max_scanid) - int(latest_num) if int(max_scanid) - int(latest_num) > 0 else 0
        if not condition:
            truncate_query = f"delete from \"{schema_name}\".\"{table_name}\" where scanid < {scanid_condition}"
        else:
            truncate_query = f"delete from \"{schema_name}\".\"{table_name}\" {condition} and scanid < {scanid_condition}"
        print(f"[*] truncate_query: {truncate_query}")
        # self.execute_sql(truncate_query)
